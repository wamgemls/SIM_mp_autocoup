...
class CouplingPlanner:
    ...
    def bilevel_check(self):
        ...
        dis_goal_trajgoal, theta_goal_trajgoal = calc_distance_angle_PoseA_PoseB(self.trajectory[-1],self.goal_pose)

        closest_trajectory_point = self.give_closestprojection_on_trajectory(self.trajectory)
        dis_ego_traj,theta_ego_traj = calc_distance_angle_PoseA_PoseB(closest_trajectory_point,self.ego_pose)
    ...
    def sample_g2straight_trajectory(self,start_pose, goal_pose):
        ...
        if calc_distance_angle_PoseA_PoseB(start_pose,goal_pose)[0] != 0.0:
    ...
    def resample_trajectory23(self, traj):
        ...
        yaw_23 = angle_interp(s_23,s_raw,yaw_raw)
    ...
    def offset_yaw_forward(self,trajectory):
        ...
        for trajectory_point in trajectory:
            trajectory_point.yaw = angle_interval(trajectory_point.yaw)

    def offset_yaw_backward(self,trajectory):

        for trajectory_point in trajectory:
            trajectory_point.yaw = angle_interval(trajectory_point.yaw-np.pi)
    ...
    def give_closestprojection_on_trajectory(self,trajectory):
        ...
        for trajpoint in trajectory:
            ...
            if calc_distance_angle_PoseA_PoseB(trajpoint,self.ego_pose)[0] < calc_distance_angle_PoseA_PoseB(closest_trajpoint,self.ego_pose)[0]:
    ...
    def give_latprojection_on_trajectory(self,trajectory):
        ...
        while i < len(trajectory):
            ...
            x,y = find_intersection(trajectory[i-1].x,trajectory[i].x,trajectory[i-1].y,trajectory[i].y,\
