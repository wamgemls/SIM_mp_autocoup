...
class RRT:
    ...
    class Node:
        ...
        def __init__(self, x, y, theta,curvature=0.0):
            self.x = x
            self.y = y
            self.theta = theta
    ...
    def tree_building(self, goal, node_tree, animation):
        ...
        while tick_planned is False and i < length:
            ...
            new_node = self.clothoid_steer2(next(nearest_node_iter), rnd_node, self.expand_dis)
            ...
            if self.check_if_outside_play_area(new_node, self.play_area) \
                ...
                if not node_already_existent:
                    ...
                    for near_node in near_nodes:
                        ...
                        new_temp_node = self.g2_steer(near_node, new_node)
                    ...
                    for near_node in near_nodes:
                        ...
                        new_temp_node = self.g2_steer(new_node, near_node)
    ...
    def gen_triangle(self, from_node, to_node, inter_node):
        ...
        if dis_fn < dis_tn:
            ...
            tempNode1 = self.g2_steer(from_node, tri_node)
            ...
            tempNode2 = self.g2_steer(tri_node,to_node)
            ...
            new_node = self.g2_steer(from_node,to_node)
            new_node.parent = from_node
    ...
    def clothoid_steer3(self, from_node,rnd_node, arc_len):
        dmin = float("inf")
        kopt = None
        new_node = None

        upper_curv_bound = self.max_curvature - from_node.curvature
        lower_curv_bound = - self.max_curvature - from_node.curvature
        #curvature constraints
        for kd in np.arange(-lower_curv_bound,upper_curv_bound,0.01):
            clot = clothoid.Clothoid.StandardParams(from_node.x, from_node.y ,from_node.theta, from_node.curvature, kd, arc_len)
            dx = clot.XEnd - rnd_node.x
            dy = clot.YEnd - rnd_node.y
            d = math.hypot(dx, dy)
            ...
            if  d < dmin:
                dmin = d
                new_node = self.Node(clot.XEnd,clot.YEnd,clot.ThetaEnd,clot.KappaEnd)
                new_node.path_x, new_node.path_y = clot.SampleXY(50)
    ...
    def g2_steer(self,from_node, to_node):
        #generating G1fitting between two nodes
        new_node = self.Node(to_node.x,to_node.y,to_node.theta,to_node.curvature)

        g2_clothoid = clothoid.SolveG2(from_node.x, from_node.y ,from_node.theta,from_node.curvature, to_node.x ,to_node.y ,to_node.theta,to_node.curvature)

        g2_clothoid_0_x,g2_clothoid_0_y = g2_clothoid[0].SampleXY(50)
        g2_clothoid_1_x,g2_clothoid_1_y = g2_clothoid[1].SampleXY(50)
        g2_clothoid_2_x,g2_clothoid_2_y = g2_clothoid[2].SampleXY(50)
        

        new_node.path_x = g2_clothoid_0_x + g2_clothoid_1_x + g2_clothoid_2_x
        new_node.path_y = g2_clothoid_0_y + g2_clothoid_1_y + g2_clothoid_2_y

        total_length = g2_clothoid[0].length + g2_clothoid[1].length + g2_clothoid[2].length

        new_node.parent = from_node

        if -self.max_curvature < g2_clothoid[0].KappaStart < self.max_curvature and -self.max_curvature < g2_clothoid[0].KappaEnd < self.max_curvature:
            if -self.max_curvature < g2_clothoid[1].KappaStart < self.max_curvature and -self.max_curvature < g2_clothoid[1].KappaEnd < self.max_curvature:
                if -self.max_curvature < g2_clothoid[2].KappaStart < self.max_curvature and -self.max_curvature < g2_clothoid[2].KappaEnd < self.max_curvature:
                    new_node.cost = total_length
                else:
                    new_node.cost = math.inf
            else:
                new_node.cost = math.inf
    ...
    def line_steer(self, from_node, to_node, expand_dis=float("inf")):
        ...
        new_node = self.Node(from_node.x, from_node.y, theta,from_node.curvature)
    ...
    @staticmethod
    def plot_arrow(x, y, yaw, length=0.15, width=0.15, fc="k", ec="k"):  # pragma: no cover
...
def main():
    ...
    rrt = RRT(
        start=[5, 17, np.deg2rad(0),0.0],
        goal=[9,3, np.deg2rad(90),0.0],
        rand_area=[-5,25],
        expand_dis=3,
